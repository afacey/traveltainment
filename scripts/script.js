...
app.getMovieData = function (type, searchFilter = {}) {
  ...
  app
    .getMediaData(path, searchFilter)
    ...
    .then((response) => {
      ...
      const mediaResultsDetail = results.map((media) =>
        app.getMediaData(`${type}/${media.id}`)
      );

      // wait for all the api calls (promises) to be completed
      $.when(...mediaResultsDetail).then((...results) => {
        const mediaDetails = results
          ...
          .filter(({ runtime, episode_run_time }) => {
            const mediaRuntime =
              type === "movie" ? runtime : episode_run_time[0];
  ...
  results.forEach(
    ({
      ...
      const imageSrc = backdrop_path
        ? app.movieImageUrl + backdrop_path
        : app.movieImageUrl + poster_path;

      const inMediaList = app.mediaList.find((media) => media.id === id);
			...
			<div class="mediaResults__container" data-id="${id}" data-type="${mediaType}" data-runtime="${mediaRuntime}">
				...
				<div class="imageContainer">
					...
					<div class="mediaResults__moreInfo">
						<a href="${
              app.infoUrl
            }${mediaType}/${id}" target="_blank">More Info<i class="fas fa-external-link-alt"></i></a>
					</div>
				</div>
				<div class="mediaResults__info">
					<p class="mediaResults__runtime">${timeString}</p>
					<button class="button__primary button__list ${
            inMediaList ? "button__list--added" : "button__list--add"
          }">${inMediaList ? "Remove" : "Add to list"}</button>
				</div>
